...
#
# - I tried to pass a Net::SSH2 connetion to Net::Telnet(::Cisco) fhopen()
...
sub open {
    ...
    # host provided
        ...
        # connect if IPv4
        if ($family eq 'ipv4') {
            $r = $ssh->connect($host, $port, Timeout => $timeout)
...
and pass that to C<connect()>.  In this modules, Net::SSH2::Cisco takes
care of IPv6 for you with the C<family()> method by essentially doing
the same thing in the background.  If for some reason you're dropping
this into existing code, you may already create your own IO object, so
...
Currenly, only C<auth_password> is supported.
